                    ...
                    {msg.file && msg.file.url && (
                      <div style={{marginTop:4}}>
                        {msg.file.url.match(/\.(jpg|jpeg|png|gif)$/i) ? (
                          <img src={`http://localhost:5000${msg.file.url}`} alt="file" style={{maxWidth:120, borderRadius:8}} />
                        ) : (
                          <a href={`http://localhost:5000${msg.file.url}`} target="_blank" rel="noopener noreferrer">Download File</a>
                        )}
                      </div>
                    )}
                    {/* Reaction picker */}
